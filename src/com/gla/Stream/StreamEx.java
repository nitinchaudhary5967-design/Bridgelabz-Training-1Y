...
//        map
        ...
        // distnict & sorted () & chaningof methods
        List<Integer> filteredList2 = list1.stream().filter(x->x%2 == 0).map(x->x*2).distinct().sorted().limit(5).skip(1).collect(Collectors.toList());
